...
export const InternshipsView: React.FC = () => {
  ...
  const allInternships = opportunities.filter(o => o.type === 'internship');

  // Filter and rank internships according to career role and B-Tech branch
  const filteredInternships = useMemo(() => {
    return allInternships
      .filter((intern) => {
        // Mode filter
        if (selectedMode !== 'All' && intern.workMode !== selectedMode) return false;
  ...
  }, [allInternships, selectedRoleId, selectedBranch, isTailoredOnly, selectedMode, minMatch, searchQuery, getOpportunityMatch]);

  // Statistics
  const tailoredInternshipsCount = allInternships.filter(intern => {
    const m = checkOpportunityRoleAndBranchMatch(intern, selectedRoleId, selectedBranch);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 p-6 sm:p-8 rounded-3xl text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 text-blue-200 text-xs font-semibold backdrop-blur-xs border border-white/10">
              ...
              Verified Pre-Placement Offers (PPO) & Summer Internships
            </div>
            <h1 className="text-2xl sm:text-3xl font-black tracking-tight">
              Industry Internship Marketplace
            </h1>
            <p className="text-xs sm:text-sm text-blue-100/80 max-w-2xl leading-relaxed">
              Internships matched directly to your selected <strong className="text-white">Career Role</strong> and verified against your <strong className="text-white">B-Tech Branch Studied</strong>.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 bg-white/10 p-3 rounded-2xl backdrop-blur-md border border-white/15">
            <div className="text-left sm:text-right pr-2">
              ...
              <span className="text-xl font-black text-white">{tailoredInternshipsCount} of {allInternships.length} Internships</span>
            </div>
            <button
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Filter internship listings tailored to your career trajectory and branch eligibility</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-xl">
            ...
            >
              ...
              Tailored for Me ({tailoredInternshipsCount})
            </button>
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-[11px] text-slate-400">
              Active track: <strong className="text-slate-600">{activeRole.title}</strong> &bull; {activeRole.domain}
            </p>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 text-xs bg-slate-50 p-3 rounded-xl border border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-600">
              Showing internships matching: <strong className="text-indigo-700">{activeRole.title}</strong> &bull; <strong className="text-sky-700">{selectedBranch === 'All' ? 'All Branches' : selectedBranch}</strong>
            </span>
          </div>
          ...
          {isTailoredOnly ? (
            ...
            <span className="text-xs font-semibold text-slate-500">
              Showing All Internships (Ranked by role & branch affinity)
            </span>
          )}
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 flex flex-col md:flex-row items-center gap-3">
        <div className="relative flex-1 w-full">
          ...
          <input
            type="text"
            placeholder="Search internships by role, company, or tech stack..."
            value={searchQuery}
      ...
      {filteredInternships.length === 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center space-y-3">
          ...
          <h3 className="text-base font-bold text-slate-800">
            No internships match "{activeRole.title}" in "{selectedBranch}"
          </h3>
          <p className="text-xs text-slate-400 max-w-md mx-auto">
            Try switching to "All Internships" or adjust your branch filter to see opportunities across adjacent disciplines.
          </p>
